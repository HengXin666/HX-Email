...
class FallbackMailProvider:
    ...
    def read_messages(
        ...
        if email_account.provider in _OUTLOOK_PROVIDERS:
            graph_msgs = self._try_graph_read(email_account)
            if graph_msgs is not None:
                return graph_msgs  # type: ignore[return-value]

        return self._imap.read_messages(email_account)  # type: ignore[return-value]
    ...
    def read_messages_with_method(
        ...
        if email_account.provider in _OUTLOOK_PROVIDERS:
            ...
            imap_msgs = self._imap.read_messages(email_account)
    ...
    def _try_graph_read(self, email_account: EmailAccountMailbox) -> list[MailboxMessage] | None:
        """Try Graph API read; returns None if Graph is unavailable or fails."""
        try:
            result = self._graph.read_messages_result(email_account)
            if result.succeeded:
