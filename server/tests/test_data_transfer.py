...
def test_user_can_export_and_import_core_data_without_cross_user_leaks(tmp_path) -> None:
    ...
    source.put("/admin/settings/registration", json={"enabled": True}, headers=admin_headers)
    ...
    group = source.post(
        "/groups", json={"name": "Register", "color": "#58a6ff"}, headers=alice_headers
    ).json()
    tag = source.post(
        "/tags", json={"name": "GitHub", "color": "#238636"}, headers=alice_headers
    ).json()
    source.put(
        f"/usable-emails/{account['usable_emails'][1]['id']}/organize",
        json={"label": "Alias", "group_id": group["id"], "tag_ids": [tag["id"]]},
        headers=alice_headers,
    )
    platform = source.post("/platforms", json={"name": "GitHub"}, headers=alice_headers).json()
    source.post(
        f"/usable-emails/{account['usable_emails'][1]['id']}/platform-bindings",
    ...
    exported = source.get("/data/export", headers=alice_headers)
    ...
    imported = target.post("/data/import", json=exported.json(), headers=target_headers)
    duplicate_import = target.post("/data/import", json=exported.json(), headers=target_headers)
    workbench = target.get("/workbench/usable-emails", headers=target_headers)
    platforms = target.get("/platforms", headers=target_headers)
    bindings = target.get(
        f"/usable-emails/{imported.json()['usable_emails'][1]['id']}/platform-bindings",
