// HX-Email Gmail 应用专用密码助手 — content script (MV3)
// 在 myaccount.google.com / accounts.google.com / mail.google.com 上注入：
//  1. 两步验证状态检测与一键开启引导（Google 强制密码/手机确认，无法全自动）
//  2. 应用专用密码页一键生成 + 自动复制 16 位密码
//  3. Gmail 设置页自动打开 IMAP 开关并保存
// 兼容方式：页面 URL 带 ?hx=auto 或 #hx=auto 时自动执行对应页面的主操作。

(() => {
  "use strict";

  const APP_NAME_DEFAULT = "HX-Email";
  const APP_PASSWORD_RE = /^([a-z]{4} ?){3}[a-z]{4}$/; // 16 位小写字母，组间可有单个空格
  ...
  function byText(root, matcher) {
    ...
    for (const el of root.querySelectorAll("button, a, [role='button'], input, label, div, span")) {
      if (el.children.length > 0) continue; // 只匹配叶子节点文本
  ...
  function findButton(matcher) {
    for (const doc of allDocuments()) {
      ...
      if (els.length) {
        return els[0].closest("button, [role='button'], a") || els[0];
      }
  ...
  async function loadSettings() {
    try {
      const res = await chrome.runtime.sendMessage({ type: "hx-get-settings" });
      return res || {};
    } catch (_err) {
      return {};
    }
  ...
  function stateAppPasswords() {
    ...
    if (extractGeneratedPassword()) return "have_password";
    if (findGenerateButton() || findNameInput()) return "ready";
    return "loading";
  }
  ...
  function findGenerateButton() {
    return findButton(
      (t) => t === "生成" || /^(生成|Generate|Create)$/i.test(t) ||
        /生成|generate|create/i.test(t),
    );
  }
  ...
  function fillCustomName(name) {
    for (const doc of allDocuments()) {
      for (const sel of doc.querySelectorAll("select")) {
        const custom = [...sel.options].find((opt) =>
          /自定义|其他|custom|other/i.test(opt.textContent || ""),
        );
        if (custom) {
  ...
  async function generateAppPassword() {
    const { appName = APP_NAME_DEFAULT } = await loadSettings();
    ...
    const dialog = await waitFor(
      () =>
        allDocuments().some((doc) =>
          doc.querySelector("[role='dialog'], [role='alertdialog']"),
        ),
      8000,
    );
    if (!dialog) {
      panel.setStatus("没有出现密码弹窗，可能已生成完成或页面结构变化。");
    }
    ...
    try {
      chrome.runtime.sendMessage({
        type: "hx-generated",
        payload: { password, email, line, copied, at: new Date().toISOString() },
      });
    } catch (_err) {
      /* 无 background 场景忽略 */
    }
  ...
  async function fixImap() {
    const enableRadio = await waitFor(
      () =>
        [...allDocuments().flatMap((doc) => [...doc.querySelectorAll("input[type='radio']")])]
          .find((el) => {
            const label = closestLabel(el) || "";
            return /启用 IMAP|Enable IMAP/i.test(label);
          }) || null,
      10000,
    );
    if (!enableRadio) {
  ...
  const panel = (() => {
    ...
    root.innerHTML = `
      <style>
        ...
        * { box-sizing: border-box; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", "PingFang SC", "Microsoft YaHei", sans-serif; }
        ...
        .head { display: flex; align-items: center; gap: 8px; padding: 10px 12px; cursor: move;
                background: #1a73e8; color: #fff; user-select: none; }
        ...
        .btn { border: 1px solid #dadce0; background: #f8f9fa; color: #1a73e8; border-radius: 8px;
        ...
        .btn.danger { color: #d93025; border-color: #f4c7c3; }
        ...
        .result .pwd { font-family: "SF Mono", Consolas, monospace; font-size: 15px; font-weight: 700;
                       letter-spacing: 1px; color: #174ea6; word-break: break-all; }
        .result .line { font-family: "SF Mono", Consolas, monospace; font-size: 11px; color: #3c4043;
                        word-break: break-all; margin-top: 6px; background: #fff; border: 1px solid #dadce0;
      ...
      <div class="panel">
        <div class="head">
          ...
          <button data-act="collapse" title="最小化">–</button>
          <button data-act="close" title="关闭">×</button>
        </div>
        <div class="body">
          <div class="status">初始化中…</div>
          <div class="row" data-row="actions"></div>
    ...
    const actionsEl = root.querySelector('[data-row="actions"]');
    ...
    // 拖拽
    root.querySelector(".head").addEventListener("mousedown", (e) => {
      dragState = { dx: e.clientX - panelEl.getBoundingClientRect().left, dy: e.clientY - panelEl.getBoundingClientRect().top };
      e.preventDefault();
    });
    window.addEventListener("mousemove", (e) => {
      if (!dragState) return;
      const right = Math.max(8, window.innerWidth - (e.clientX - dragState.dx + panelEl.offsetWidth));
      const bottom = Math.max(8, window.innerHeight - (e.clientY - dragState.dy + panelEl.offsetHeight));
    ...
    root.querySelector('[data-act="collapse"]').addEventListener("click", () => {
      ...
      root.querySelector('[data-act="collapse"]').textContent = collapsed ? "+" : "–";
    });
    ...
    function showResult(password, line, email, copied) {
      resultEl.innerHTML = `
        <div class="result">
          ...
          <div class="row" style="margin-top:8px">
            <button class="btn" data-act="copy-pwd">复制密码</button>
            <button class="btn" data-act="copy-line">复制导入行</button>
            <button class="btn primary" data-act="open-hx">打开 HX-Email</button>
          </div>
        </div>
      `;
      resultEl.querySelector('[data-act="copy-pwd"]').addEventListener("click", async () => {
        const ok = await copyText(password);
        setStatus(ok ? "✓ 密码已复制" : "复制失败，请手动复制。", "ok");
      });
      resultEl.querySelector('[data-act="copy-line"]').addEventListener("click", async () => {
      ...
      resultEl.querySelector('[data-act="open-hx"]').addEventListener("click", () => {
        try {
          chrome.runtime.sendMessage({
            type: "hx-generated",
            payload: { password, email, line, copied, openHxEmail: true, at: new Date().toISOString() },
          });
        } catch (_err) {
          /* ignore */
        }
    ...
    return { setStatus, setActions, setHint, showResult };
  })();
  ...
  function bootAppPasswords() {
    let autoRan = false;
    panel.setActions([
      { label: "一键生成应用专用密码", primary: true, onClick: () => void generateAppPassword() },
      { label: "去开启两步验证", onClick: () => void chrome.runtime.sendMessage({ type: "hx-open-url", url: URLS.twoStep }) },
      { label: "检查 IMAP", onClick: () => void chrome.runtime.sendMessage({ type: "hx-open-url", url: URLS.imap }) },
    ]);
    panel.setHint("默认使用「HX-Email」作为应用名称；可在插件弹窗中修改。生成的密码不会自动过期，除非在 Google 后台撤销。");

    const tick = async () => {
      const state = stateAppPasswords();
      if (state === "login") {
        panel.setStatus("检测到未登录 Google 账号。请先登录后重新打开此页。", "warn");
        panel.setActions([
          { label: "打开登录页", primary: true, onClick: () => void chrome.runtime.sendMessage({ type: "hx-open-url", url: URLS.login }) },
        ]);
        return;
      }
      if (state === "twostep_off") {
        panel.setStatus("⚠ 两步验证未开启，无法生成应用专用密码。点击下方按钮去开启（需完成 Google 的密码/手机验证）。", "warn");
        panel.setActions([
          { label: "去开启两步验证", primary: true, onClick: () => void chrome.runtime.sendMessage({ type: "hx-open-url", url: URLS.twoStep }) },
        ]);
        if (AUTO && !autoRan) {
          autoRan = true;
          await sleep(600);
          chrome.runtime.sendMessage({ type: "hx-open-url", url: URLS.twoStep });
        }
        return;
      }
      if (state === "have_password") {
      ...
      if (state === "ready") {
        panel.setStatus("表单已就绪，点击按钮即可自动生成并复制 16 位应用专用密码。");
        if (AUTO && !autoRan) {
          autoRan = true;
  ...
  function bootTwoStep() {
    let autoRan = false;
    panel.setActions([
      { label: "打开应用专用密码页", primary: true, onClick: () => void chrome.runtime.sendMessage({ type: "hx-open-url", url: URLS.appPasswords }) },
    ]);
    ...
    const tick = async () => {
      ...
      if (bodyHasText(/已开启|开启.?状态|Turned on|On(?!-)/i) && !findButton((t) => /开始设置|Get started/i.test(t))) {
        panel.setStatus("✓ 两步验证已开启。去生成应用专用密码吧。", "ok");
        return;
      }
      if (findButton((t) => /开始设置|Get started|Turn on 2-?Step/i.test(t))) {
        panel.setStatus("两步验证尚未开启。点击「开启」后请完成 Google 的密码/手机验证（无法自动完成）。", "warn");
        panel.setActions([
          { label: "开启两步验证", primary: true, onClick: () => void handleTwoStepOff() },
          { label: "去应用专用密码页", onClick: () => void chrome.runtime.sendMessage({ type: "hx-open-url", url: URLS.appPasswords }) },
        ]);
        if (AUTO && !autoRan) {
          autoRan = true;
          await handleTwoStepOff();
        }
        return;
      }
  ...
  function bootImap() {
    panel.setActions([
      { label: "一键开启 IMAP 并保存", primary: true, onClick: () => void fixImap() },
      { label: "去生成应用密码", onClick: () => void chrome.runtime.sendMessage({ type: "hx-open-url", url: URLS.appPasswords }) },
    ]);
    panel.setHint("个人 Gmail 通常默认启用 IMAP；此步骤仅在关闭时自动打开并保存。");
    const tick = () => {
      const radio = [...allDocuments().flatMap((doc) => [...doc.querySelectorAll("input[type='radio']")])]
        .find((el) => /启用 IMAP|Enable IMAP/i.test(closestLabel(el)));
      if (radio) {
        if (radio.checked) {
          panel.setStatus("✓ IMAP 已启用，无需修改。", "ok");
        } else {
          panel.setStatus("IMAP 当前为关闭状态，点击「一键开启 IMAP 并保存」。", "warn");
        }
        return;
      }
  ...
  function bootLogin() {
    ...
    panel.setActions([
      { label: "打开登录页", primary: true, onClick: () => void chrome.runtime.sendMessage({ type: "hx-open-url", url: URLS.login }) },
      { label: "打开应用专用密码页", onClick: () => void chrome.runtime.sendMessage({ type: "hx-open-url", url: URLS.appPasswords }) },
    ]);
  }

  function bootOther() {
    panel.setStatus("此页面暂不支持。请使用插件弹窗中的功能按钮。");
    panel.setActions([
      { label: "打开应用专用密码页", primary: true, onClick: () => void chrome.runtime.sendMessage({ type: "hx-open-url", url: URLS.appPasswords }) },
    ]);
  }
