...
完整、最新的数据镜像。同步方向是**主 → 从**（单向拉取），从机**只增不删**
（主实例删除的数据不会从从机删除），按自然键去重，保证不重复、不丢失。
...
- 从机启动服务后立即同步一轮，之后按间隔周期同步（后台线程，失败不影响服务）；
- 不启动服务也能手动同步一轮：`hx-email sync`（需同样配置好环境变量）；
