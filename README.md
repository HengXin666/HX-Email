...
- **私网/内网地址**：默认放行 RFC1918 私网段（`10.x` / `172.16-31.x` / `192.168.x`）、ULA 地址与 Clash/mihomo fake-ip 段（`198.18.0.0/15`，OpenClash/软路由的 fake-ip DNS 会把所有域名解析进该段），LAN 代理（如 `http://192.168.1.50:7890`）或 Docker 桥接网关 IP 可直接填写；仅拦截云元数据（`169.254.169.254`）、link-local、组播、TEST-NET 等危险保留段。若部署在公网且面向多租户，可在 `.env` 设置 `HX_EMAIL_ALLOW_PRIVATE_PROXY=false` 收紧为白名单（仅 `127.0.0.0/8`、`::1`、`host.docker.internal` 与公网地址）。
