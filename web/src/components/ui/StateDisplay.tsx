...
export const EmptyState: React.FC<EmptyStateProps> = ({ message = "暂无数据", className = "" }) => (
  <div className={`text-center py-16 text-gh-text-secondary text-sm ${className}`}>{message}</div>
);
...
export const ErrorState: React.FC<ErrorStateProps> = ({ message, onRetry, className = "" }) => (
  <div className={`flex flex-col items-center justify-center py-16 gap-3 ${className}`}>
    ...
    {onRetry && (
      <button onClick={onRetry} className="text-xs text-gh-accent hover:underline">
        重试
      </button>
    )}
  </div>
