...
import { maskValue, CRON_PRESETS } from './impl/settings_api'
...
const BasicTab: React.FC<TabProps> = ({ settings, setSetting, toast, user }) => {
  ...
  const handleAITest = async () => {
    ...
    try {
      ...
      setAiTestResult({ success: res.success, message: res.message || (res.code ? `Code: ${res.code}` : '已测试') })
    } catch (err: any) {
    ...
    >
      ...
      <Card className="p-5">
        ...
        <div className="space-y-3 max-w-md">
          <Input label="当前密码" type="password" value={curPwd} onChange={(e) => setCurPwd(e.target.value)} />
...
const TempMailTab: React.FC<TabProps> = ({ settings, setSetting, toast }) => {
  ...
  const handleSyncDomains = async () => {
    ...
    if (!workerUrl || !adminKey) { toast('请先填写 Worker URL 和 Admin Key', 'error'); return }
    setSyncLoading(true)
    setSyncResult(null)
    try {
      ...
      <Card className="p-5">
        ...
        <div className="space-y-3 max-w-lg">
          <div className="flex items-center gap-3">
            ...
            <Button variant="ghost" size="sm" onClick={() => handleCopyKey(extApiKey)} disabled={!extApiKey}>
              <IconCopy size={13} />
            </Button>
            <Button variant="secondary" size="sm" onClick={handleRevealPlaintext} loading={revealLoading}>
          ...
          {showPlaintext && plaintextKey && (
            ...
            >
              <IconKey size={12} />
              明文 Key: {plaintextKey}
              <button onClick={() => handleCopyKey(plaintextKey)} className="ml-auto p-0.5 hover:text-gh-text">
                <IconCopy size={11} />
              </button>
            </motion.div>
          )}
...
const AutomationTab: React.FC<TabProps> = ({ settings, setSetting, toast }) => {
  ...
  const [whTestResult, setWhTestResult] = useState<{ success: boolean; message: string } | null>(null)
  ...
  const handleValidateCron = async () => {
    ...
    if (!cron) { toast('请输入 Cron 表达式', 'error'); return }
    setCronLoading(true)
    setCronResult(null)
    try {
  ...
  const handleEmailTest = async () => {
    ...
    if (!recipient) { toast('请填写收件邮箱', 'error'); return }
  ...
  const handleTelegramTest = async () => {
    ...
    if (!botToken || !chatId) { toast('请填写 Bot Token 和 Chat ID', 'error'); return }
    setTgTestLoading(true)
    setTgTestResult(null)
    try {
      const res = await api.testTelegram({ bot_token: botToken, chat_id: chatId, proxy_url: proxyUrl })
  ...
  const handleWebhookTest = async () => {
    ...
    if (!url) { toast('请填写 Webhook URL', 'error'); return }
    ...
    } finally { setWhTestLoading(false) }
  }
    ...
    >
        ...
        <div className="space-y-3 max-w-lg">
          ...
          <div>
            <div className="flex gap-2 items-end">
              ...
              <Button variant="secondary" size="sm" onClick={handleValidateCron} loading={cronLoading}>
      ...
      <Card className="p-5">
        ...
        <div className="space-y-3 max-w-lg">
          ...
          <div className="flex gap-2 items-end">
            ...
            <Button variant="secondary" size="sm" onClick={handleEmailTest} loading={emailTestLoading}>
      ...
      <Card className="p-5">
        ...
        <div className="space-y-3 max-w-lg">
          ...
          />
          <div className="flex gap-2">
            <Button variant="secondary" size="sm" onClick={handleTelegramTest} loading={tgTestLoading}>
      ...
      <Card className="p-5">
        ...
        <div className="space-y-3 max-w-lg">
          ...
          />
          <div className="flex gap-2">
            <Button variant="secondary" size="sm" onClick={handleWebhookTest} loading={whTestLoading}>
      ...
      <Card className="p-5">
        ...
        <div className="space-y-3 max-w-lg">
          ...
          <div className="flex gap-2 flex-wrap">
            <Button variant="secondary" size="sm" onClick={handleWatchtowerTest} loading={wtLoading}>
