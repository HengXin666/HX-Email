...
const TABS: Array<{
  ...
  { id: 'page-token', label: '页面 Token', description: '通过页面生成和保存 token', icon: IconLink },
  { id: 'api-doc', label: 'API 说明', description: '通过 API 换取 token 并添加账号', icon: IconCode }
]
...
const InfoItem: React.FC<{ n: string; title: string; children: React.ReactNode }> = ({ n, title, children }) => (
      ...
      <Card className="p-5">
        ...
        <div className="space-y-3">
          <InfoItem n="1" title="配置 OAuth 参数">
            填写 Client ID、Redirect URI、Scope。Tenant 固定为 consumers，适用于个人 Microsoft / Outlook 账号。
          </InfoItem>
          <InfoItem n="2" title="生成授权链接">
            点击“生成授权链接”后页面只展示链接，不再自动打开新窗口。复制链接后自行在浏览器中访问并授权。
          </InfoItem>
          <InfoItem n="3" title="粘贴回调 URL 换取 Token">
            授权完成后，把浏览器跳转到的完整 callback URL 粘贴回来，页面会通过后端 API 换取 refresh_token。
          </InfoItem>
        ...
        {prepared && (
          <div className="mt-4 rounded-lg border border-gh-accent/25 bg-gh-accent/5 p-3">
            <div className="flex items-center justify-between gap-2 mb-2">
              ...
              <Button variant="secondary" size="sm" onClick={() => copyText(prepared.authorize_url, '授权链接')}>
        ...
        {tokens && (
          <div className="mt-5 space-y-3 rounded-lg border border-gh-border bg-gh-canvas-inset p-4">
            <div className="grid grid-cols-[1fr_auto] gap-2 items-end">
              ...
              <Button variant="secondary" onClick={() => copyText(tokens.refresh_token, 'Refresh Token')}>
                <IconCopy size={13} /> 复制
              </Button>
            </div>
    ...
    <div className="flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden">
      ...
      <div className="flex-1 overflow-auto">
        <div className="max-w-5xl mx-auto p-6 space-y-5">
          ...
          {activeTab === 'guide' ? renderGuide() : activeTab === 'page-token' ? renderPageToken() : renderApiDoc()}
        </div>
      </div>
    </div>
  )
