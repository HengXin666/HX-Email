...
const PROVIDER_STORAGE_KEY = "hx_token_tool_provider";
...
function getStoredProvider(): TokenProvider {
  try {
    return window.localStorage?.getItem(PROVIDER_STORAGE_KEY) === "google" ? "google" : "microsoft";
  } catch {
    return "microsoft";
  }
}

function storeProvider(provider: TokenProvider): void {
  try {
...
export const TokenTool: React.FC = () => {
  ...
  const [provider, setProviderState] = useState<TokenProvider>(() => getStoredProvider());
  ...
  const [accounts, setAccounts] = useState<TokenToolAccount[]>([]);
  ...
  const outlookAccounts = useMemo(
    () => accounts.filter((account) => account.provider === "outlook"),
    [accounts],
  );
  ...
  const setProvider = (nextProvider: TokenProvider): void => {
